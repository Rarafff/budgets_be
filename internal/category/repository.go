...
SELECT id::text, user_id::text, name, type, icon, created_at
FROM categories
WHERE user_id = $1
...
`, userID)
	...
	for rows.Next() {
		var item Category
		if err := rows.Scan(&item.ID, &item.UserID, &item.Name, &item.Type, &item.Icon, &item.CreatedAt); err != nil {
			return nil, err
		}
...
VALUES ($1, $2, $3, $4)
RETURNING id::text, user_id::text, name, type, icon, created_at
`, userID, req.Name, req.Type, req.Icon).Scan(&item.ID, &item.UserID, &item.Name, &item.Type, &item.Icon, &item.CreatedAt)
	return item, err
}
